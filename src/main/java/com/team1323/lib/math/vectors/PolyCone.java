...
public class PolyCone extends Surface {
	...
	public class PlaneSegment extends Surface {
		...
		public PlaneSegment(Translation2d p0, Translation2d p1, Translation2d p2, double radius) {
			...
			f_ = (here -> {
				...
				if (here.isWithinAngle(p1, p0, p2) && here.distanceToLine(p1, p2) <= radius)
					return (a * (p0.x() - here.x()) + b * (p0.y() - here.y())) / c - 1.0;
				else
					return 0.0;
			});
			dfdx_ = (here -> {
		...
		public Function<Translation2d, Double> f() {
			return f_;
		}

		public Function<Translation2d, Double> dfdx() {
			return dfdx_;
		}

		public Function<Translation2d, Double> dfdy() {
			return dfdy_;
		}
	...
	public Function<Translation2d, Double> dfdx() {
		return (here -> dfdx(here));
	}

	public Function<Translation2d, Double> dfdy() {
		return (here -> dfdy(here));
	}
